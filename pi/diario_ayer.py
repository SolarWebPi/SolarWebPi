...
try:
    sql="SELECT DATE(Fecha) FROM diario WHERE DATE(Fecha) = SUBDATE(CURDATE(),1)"
    cursor.execute(sql)
    ...
    if nreg==0:
        try:
            ...
            db.commit()    
            log='Registro diario de ayer creado'
            logBD()

        except:
            db.rollback()
            log='Error en la creacion del registro diario de ayer'
            logBD()
    else:
        try:
            ...
            db.commit()
            log='Registro diario de ayer actualizado'
            logBD()
